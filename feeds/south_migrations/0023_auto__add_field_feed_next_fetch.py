...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Feed.next_fetch'
        db.add_column(u'feeds_feed', 'next_fetch',
                      self.gf('django.db.models.fields.DateTimeField')(null=True, blank=True),
                      keep_default=False)
        for f in orm.Feed.objects.all():
            now=timezone('utc').localize(datetime2.datetime.utcnow())
            f.next_fetch=now + datetime2.timedelta(minutes=f.update_interval)
            f.save()


    def backwards(self, orm):
        # Deleting field 'Feed.next_fetch'
        db.delete_column(u'feeds_feed', 'next_fetch')
