...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding field 'Feed.date_added'
        db.add_column(u'feeds_feed', 'date_added',
                      self.gf('django.db.models.fields.DateTimeField')(null=True, blank=True),
                      keep_default=False)

        # Adding field 'Feed.statistics'
        db.add_column(u'feeds_feed', 'statistics',
                      self.gf('jsonfield.fields.JSONField')(default='', blank=True),
                      keep_default=False)

        # Adding field 'Feed.statistics_updated'
        db.add_column(u'feeds_feed', 'statistics_updated',
                      self.gf('django.db.models.fields.DateTimeField')(null=True, blank=True),
                      keep_default=False)
        db.commit_transaction()

        db.start_transaction()
        orm.Feed.update(statistics={})
        for feed in orm.Feed.objects.all():
            try:
                feed.date_added=orm.Article.objects.filter(feed=feed).order_by('date_added')[0].date_added
            except:
                feed.date_added=datetime.datetime.utcnow()
            data={}
            i=0
            for article in orm.Article.objects.filter(feed=feed).only('date_published'):
            ...
            feed.statistics_updated=datetime.datetime.utcnow()
            feed.save()
        db.commit_transaction()

    def backwards(self, orm):
        # Deleting field 'Feed.date_added'
        db.delete_column(u'feeds_feed', 'date_added')

        # Deleting field 'Feed.statistics'
        db.delete_column(u'feeds_feed', 'statistics')

        # Deleting field 'Feed.statistics_updated'
        db.delete_column(u'feeds_feed', 'statistics_updated')
    ...
    models = {
        ...
        u'feeds.feed': {
            ...
            'statistics': ('jsonfield.fields.JSONField', [], {'blank': 'True'}),
