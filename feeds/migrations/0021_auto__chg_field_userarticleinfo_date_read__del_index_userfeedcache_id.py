...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Changing field 'UserArticleInfo.date_read'
        db.alter_column(u'feeds_userarticleinfo', 'date_read', self.gf('django.db.models.fields.DateTimeField')(null=True))
        # Removing index on 'UserFeedCache', fields ['id']
        db.delete_index(u'feeds_userfeedcache', ['id'])

        u=orm['auth.User'].objects.get(pk=1)
        for a in orm.Feed.objects.all():
            orm.UserFeedSubscription(user=u, feed=a).save()
        for a in orm.Article.objects.all():
            b=orm.UserArticleInfo(user=u, feed=a.feed, article=a, read=a.read)
            if a.read:
                b.date_read=timezone('utc').localize(datetime.datetime.utcnow())
            b.save()
        for a in orm.UserFeedSubscription.objects.all():
            b=orm.UserFeedCache(user=u, feed=a.feed)
            b.unread=orm.UserArticleInfo.objects.filter(user=u, feed=a.feed, read=False).count()
            b.save()


    def backwards(self, orm):
        # Adding index on 'UserFeedCache', fields ['id']
        db.create_index(u'feeds_userfeedcache', ['id'])


        # User chose to not deal with backwards NULL issues for 'UserArticleInfo.date_read'
        raise RuntimeError("Cannot reverse this migration. 'UserArticleInfo.date_read' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration
        # Changing field 'UserArticleInfo.date_read'
        db.alter_column(u'feeds_userarticleinfo', 'date_read', self.gf('django.db.models.fields.DateTimeField')())
    ...
    models = {
        ...
        u'feeds.userarticleinfo': {
            'Meta': {'ordering': "['-article__date_published', '-article__date_added', '-article__id']", 'object_name': 'UserArticleInfo', 'index_together': "[['user', 'feed'], ['user', 'feed', 'read']]"},
